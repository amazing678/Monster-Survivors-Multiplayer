...
using OctoberStudio.Save; // 引入保存系统命名空间
...
namespace OctoberStudio
{
    ...
    public class EnemiesSpawner : MonoBehaviour
    {
        ...
        // 新增检测目标的方法
        private void CheckEnemyKillGoal(StageSave stageSave)
        {
            // 当击杀数达到1500且尚未完成时
            if (enemiesDiedCounter >= 3000 && stageSave.IsPlaying)
            {
                // 解锁下一关：如果当前关卡不是最后一关，更新已解锁的最大关卡ID
                if (stageSave.MaxReachedStageId < stageSave.SelectedStageId + 1 && stageSave.SelectedStageId + 1 < StageController.Database.StagesCount)
                {
                ...
                GameController.SaveManager.Save(true); // 修正点3：使用GameController的SaveManager

                // 隐藏游戏界面，显示完成界面（修正点4：使用StageController的公开引用）
                StageController.GameScreen.Hide();
                StageController.StageCompletedScreen.Show(); // 修正点5：使用正确的公开属性
                Time.timeScale = 0;
            }
        }
