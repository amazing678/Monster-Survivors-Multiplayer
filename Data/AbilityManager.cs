...
namespace OctoberStudio.Abilities
{
    ...
    public class AbilityManager : MonoBehaviour
    {
        ...
        {
            AddAbility(abilityData, PlayerBehavior.Player1, level);
        }

        //全新的双人发牌接口
        public void AddAbility(AbilityData abilityData, PlayerBehavior targetPlayer, int level = 0)
        {
            if (targetPlayer == null) targetPlayer = PlayerBehavior.Player1;

            // 核心：把生成的技能挂在对应的玩家身上！
            IAbilityBehavior ability = Instantiate(abilityData.Prefab, targetPlayer.transform).GetComponent<IAbilityBehavior>();
        ...
        private void ShowWeaponSelectScreen()
        {
            ...
            {
                // 随机获取一个武器能力
                var randomAbility = weaponAbilities[Random.Range(0, weaponAbilities.Count)];
                // 直接添加能力，无需弹窗
                AddAbility(randomAbility);
            }
        }

        public void GiveRandomAbility(int level, PlayerBehavior targetPlayer)
        {
            ...
            if(selectedAbilities.Count > 0)
            {
                ...
                // 随机选择一个选项（0-2之间的索引）
                int randomIndex = Random.Range(0, selectedAbilities.Count);
                AbilityData randomAbility = selectedAbilities[randomIndex];

                // 在 AbilityManager.cs 的升级逻辑中（原随机选择后的升级部分）
                if (IsAbilityAquired(randomAbility.AbilityType))
                {
                    // 升级已有能力
                    int abilityLevel = save.GetAbilityLevel(randomAbility.AbilityType);

                    // 关键修改：通过等级配置数组的长度计算最大等级（假设数组名为 Levels）
                    int maxLevel = 0;
                    if (randomAbility.Levels != null) // 确保数组存在，避免空引用错误
                    {
                        maxLevel = randomAbility.Levels.Length - 1; // 数组索引从0开始，最大等级 = 长度-1
                    }
                    else
                    {
                        Debug.LogError($"能力 {randomAbility.AbilityType} 未配置等级数据！");
                        return; // 避免后续错误
                    }

                    // 仅在未达最大等级且非终局能力时升级
                    if (!randomAbility.IsEndgameAbility && abilityLevel < maxLevel)
                    {
                        abilityLevel++;
                    }

                    // 确保等级不为负
                    abilityLevel = Mathf.Max(abilityLevel, 0);

                    save.SetAbilityLevel(randomAbility.AbilityType, abilityLevel);
                    randomAbility.Upgrade(abilityLevel); // 触发升级效果
                }
                else
                {
                    // 添加新能力
                    //AddAbility(randomAbility);
                    // 核心修改：生成新技能时，明确告诉管家给哪个玩家！
                    AddAbility(randomAbility, targetPlayer, 0);
                }
            }
        }
