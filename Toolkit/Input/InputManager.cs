...
namespace OctoberStudio.Input
{
    public class InputManager : MonoBehaviour, IInputManager
    {
        ...
        // 新增：P键动作
        private InputAction toggleJoystickAction;
        // 新增：记录摇杆是否被禁用
        private bool isJoystickManuallyDisabled = false;
        ...
        public void Init()
        {
            ...
            // 新增：初始化P键动作
            toggleJoystickAction = inputAsset.Gameplay.ToggleJoystick; // 关键修正
            if (toggleJoystickAction != null)
            {
                toggleJoystickAction.performed += OnPToggled;
                toggleJoystickAction.Enable();
            }
        }

        // 新增：处理P键按下事件
        private void OnPToggled(InputAction.CallbackContext context)
        {
            if (context.performed && Joystick != null)
            {
                isJoystickManuallyDisabled = !isJoystickManuallyDisabled;

                if (isJoystickManuallyDisabled)
                {
                    Joystick.Disable();
                    Joystick.Hide(); // 隐藏摇杆UI
                }
                else
                {
                    // 只有在当前输入类型是UIJoystick时才重新启用
                    if (ActiveInput == InputType.UIJoystick)
                    {
                        Joystick.Enable();
                        Joystick.Show(); // 显示摇杆UI
                    }
                }
            }
        }
        ...
        private void Update()
        {
                ...
                Touchscreen.current != null && Touchscreen.current.wasUpdatedThisFrame))
            {
                ...
                if (Joystick != null && !isJoystickManuallyDisabled)
                    Joystick.Enable();
