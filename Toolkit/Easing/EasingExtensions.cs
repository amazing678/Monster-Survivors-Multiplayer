...
namespace OctoberStudio.Easing
{
    public static class EasingExtensions
    {
        ...
        public static IEasingCoroutine DoPosition(this Transform transform, Vector3 targetPosition, float duration, float delay = 0)
        {
            ...
            // 保存当前transform的引用（避免闭包中捕获的transform被销毁后仍访问）
            Transform targetTransform = transform;
            return new VectorEasingCoroutine3(startPosition, targetPosition, duration, delay, (position) =>
            {
                // 关键：检查transform是否还存在（未被销毁）
